...
const QRModal = ({ qrModalVisible, setQrModalVisible, selectedQrData }) => {
  const qrLabelRef = useRef();

  if (!selectedQrData) {
    console.log("selectedQrData----------")
    return null; // Prevent rendering
  }
  const scannedValue = selectedQrData.childPartScan; // spaces are important, do not trim
  ...
  const data = {
    ...
    itemNo: childPartCode,
    deliveryDate: deliveryDate
  ? `D${deliveryDate.replaceAll("-", "")}`
  : "",

manufactureDate:productionDate
  ? `P${productionDate.replaceAll("-", "")}`
  : "",
    ...
    pkgNo: labelNumber,
    batchNo: batchNumber,
    traceability:scannedValue || "QR123"
  };
