...
import PDF417Barcode from "../Reports/Picklist/PDF417Barcode";
import { toast } from "react-toastify";
import html2canvas from "html2canvas";
import jsPDF from "jspdf";

const LabelGenrationQRModel = ({ qrModalVisible, setQrModalVisible, selectedQrData }) => {
  const qrLabelRef = useRef(null);
  if (!selectedQrData) {
    return null; // Prevent rendering
  }
  const scannedValue = selectedQrData.traceabilityCode; // spaces are important, do not trim
  ...
  const data = {
    consignee: `${selectedQrData?.consignee}`,
    unloadingPoint: `${selectedQrData?.unloadingPoint}`,
    deliveryNoteNo: invoiceNumber || "",
    itemNo: `${selectedQrData?.customerItemNo}`,
    ...
    description: selectedQrData?.itemDescription,
    quantityFilled: lineQt,
    packageRefNo:  `${selectedQrData?.packageReferenceNo}`,
    supplierNo: vendorCode,
    pkgNo: selectedQrData?.pkgNo,
    batchNo: batchNumber,
...
export default LabelGenrationQRModel;